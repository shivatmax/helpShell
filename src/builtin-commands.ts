...
export const builtinCommands: AICommand[] = [
  {
    ...
    require_stdin: true,
    prompt:
      "Generate git commit message following Conventional Commits specification based on the git diff output in stdin\nYou must return a commit message only. Try to capture the key changes in the title in under 72 characters. For changes that are not captured in the title and are important for the developer to know, add the dehoils in the body as a list. Do not add dehoils if the title is already clear. Do not output any other text or quotes.",
  },
]
